...
pub fn col(db: &Database) -> mongodb::Collection<TransportDoc> {
    db.collection("transports")
}

pub async fn insert(db: &Database, doc: &TransportDoc) -> Result<()> {
    col(db).insert_one(doc, None).await?;
    Ok(())
}

pub async fn find(db: &Database, user_id: &ObjectId, name: &str) -> Result<Option<TransportDoc>> {
    Ok(col(db)
        .find_one(doc! { "user_id": user_id, "name": name }, None)
        .await?)
}

pub async fn list(db: &Database, user_id: &ObjectId) -> Result<Vec<TransportDoc>> {
